...
for fold_id, (trn_idx, val_idx) in enumerate(kfold.split(df)):
    ...
    train_model(model, trainloader, validloader, 'kfold_tmp_cnnavg_' + ("large" if args.zhlarge else "small"))
    del model
    torch.cuda.empty_cache()

    model = Bert_Plus_TextCNN_WordAvg.from_pretrained(PRETRAINED_MODEL_PATH, num_labels=NUM_LABELS)
    model.load_state_dict(torch.load('model_output/' + 'kfold_tmp_cnnavg_' + ("large" if args.zhlarge else "small")))
...
df_pred = pd.concat([df.loc[:, ["微博id"]], 
                            df_out.loc[:, 'y']], axis=1)
df_pred.to_csv('kfold_cnnavg_' + ("large" if args.zhlarge else "small") + '-final.csv', index=None)
