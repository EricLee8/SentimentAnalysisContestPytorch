...
def get_filter_masks(text, filter_sizes=FILTER_SIZES):
    ...
    for filter_size in filter_sizes:
        ...
        for i in range(len(text)-filter_size+1):
            ...
            filter_masks[-1].append(1 if word in words_list else 0.5)
...
def train_model(model, trainloader, validloader):
    ...
    for epoch in range(EPOCHS):
        ...
        print('[epoch %d], valid_acc: %.3f' %
              (epoch + 1, acc))
        if acc >= best_acc:
            print("Best accuracy, saving model to disk...")
            best_acc = acc
            torch.save(model.state_dict(), 'model_output/' + 'textcnn_with_attention_kmax.pth')
...
def main():
    ...
    model.load_state_dict(torch.load('model_output/' + 'textcnn_with_attention_kmax.pth'))
