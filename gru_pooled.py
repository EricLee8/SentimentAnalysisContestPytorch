...
MODEL_NAME = 'gru_pooled.pth' if not args.zhlarge else "gru_pooled_large.pth"
...
class Bert_GRU_Pooled(BertPreTrainedModel):
    def __init__(self, config, gru_hidden_size=768//2):
        ...
        self.L_gru = nn.Linear(gru_hidden_size*2 + config.hidden_size, self.config.num_labels)
    ...
    def forward(self, input_ids=None, attention_mask=None, token_type_ids=None, position_ids=None,
        ...
        catted = torch.cat((pooled_output, gru_pooled), dim=1)
        gru_logits = self.L_gru(catted)
        
        # logits = avg_logits + gru_logits
        logits = gru_logits
...
def gen_res(model, tokenizer):
    ...
    df_pred = pd.concat([df.loc[:, ["微博id"]],
                            df_out.loc[:, 'y']], axis=1)
    df_pred.to_csv('gru_pooled-final.csv', index=False)
