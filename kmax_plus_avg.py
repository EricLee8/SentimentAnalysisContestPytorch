...
class Bert_Plus_TextCNN(BertPreTrainedModel):
    def __init__(self, config, filter_sizes=FILTER_SIZES, num_filters=192//K):
...
def get_filter_masks(text, filter_sizes=FILTER_SIZES):
    words_list = list(jieba.cut(text, cut_all=True))
    filter_masks = []
    for filter_size in filter_sizes:
        ...
        for i in range(len(text)-filter_size+1):
            ...
            filter_masks[-1].append(1 if word in words_list else 0.6)
...
def gen_res(model, tokenizer):
    ...
    df_pred = pd.concat([df.loc[:, ["微博id"]],
                            df_out.loc[:, 'y']], axis=1)
    df_pred.to_csv('kmaxAvg-final.csv', index=False)


# 訓練模式
def train_model(model, trainloader, validloader, model_name='textcnn_with_attention_kmax_plus_wordavg.pth'):
...
def main():
    ...
    if args.gen_res:
        ...
        model.load_state_dict(torch.load('model_output/' + 'textcnn_with_attention_kmax_plus_wordavg.pth'))
